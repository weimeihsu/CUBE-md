...
4. **Icons** — `icons.md` — utility icon sets; must precede **Device Layout** (its
   browser chrome uses `i-05`/`i-06`/`i-19`) and the components that reference icons.
5. **Device Layout** — `device-layout.md` — **install last of the Base**. Page
   **`6 Device Layout`** now contains the device frames **and** their (simplified)
   headers/footers as local frames; it references Spacing, Colors, Typography and Icons.

> ⚠️ **`logos.md` is NOT in the rebuild workflow; the separate Header & Footer doc has been removed.**
> - **Logos** (page `4 Logos`) proved too complex to rebuild reliably and is excluded;
>   the headers/footers now use a plain **`gray/gray150` square as a logo placeholder**,
>   so `logos.md` is not needed. (The `4 Logos` page still exists in the source but is
>   not rebuilt.)
> - **Header & Footer** was **merged into `6 Device Layout`** (simplified) and its
>   separate doc + reference-image folder were deleted — build the headers/footers
>   **only from `device-layout.md`**. Do not look for a `header-footer.md` or a
>   `HeaderFooter/` folder; they no longer exist.

> 🛑 **Checkpoint — after Step 5 (all Base foundations done), STOP and ask the user**
...
> link remote/published-library values.
...
複雜元件（如 `device-layout.md` 的五種畫板與其頁首／頁尾、`icons.md` 的大量 SVG）在單一
回合內，agent 常因複雜度**壓縮或略過子節點**。這是可以接受的——請採**兩階段**，不要為了
塞進一回合而永久省略細節：
